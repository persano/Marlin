...
**Firmware:** `firmware-gpro-v9-0x08000000.bin`
...
| **Our v9** | `releases/v9/firmware-gpro-v9-0x08000000.bin` |
...
| Fan kickstart (spin-up burst) | no | no | no | **yes** (100 ms) |
...
| M412 | Filament runout enable / distance | no | no | no | **yes** |
...
| Input Shaping X+Y (M593) | no | no | yes | yes |
...
| FT Motion trajectory type | — | — | — | TRAPEZOIDAL @ 1 kHz |
...
| Filament runout sensor (M412) | no | no | no | **yes** (PA0, M600 trigger) |
...
| Parenthesis comments support | no | no | no | **yes** |
| Quoted string parameters | no | no | no | **yes** |
...
| M73 progress reporting | no | no | yes | no |
| Startup auto-temp push (M155) | no | no | **no** | **yes** |
| Preheat temps broadcast to TFT | no | no | no | **yes** |
...
All three custom firmwares add: BLTouch + UBL, dual serial for TFT, hardware-tuned motion/PID, input shaping, linear advance, arc support, power loss recovery (except mfagp), host communication, cancel objects, and a large set of G-codes absent from a generic build. v9 further adds FT Motion (M493) and filament runout sensor support.

### vs gpro-mp

Both mfagp and our v9 add over gpro-mp: M92, M211, input shaping, linear advance, arc support, cancel objects, full host action commands, hardware watchdog, serial overrun protection, faster parser, busy-while-heating, host keepalive, QUICK_HOME, VALIDATE_HOMING_ENDSTOPS, SD features, M73 progress, M48 probe test. v9 additionally adds FT Motion and filament runout sensor.

### vs mfagp (Marlin-for-artillery-genius-pro-bugfix-2.1.x)

**mfagp has, our v9 does not:**
...
**Our v9 has, mfagp does not:**

| Feature | Effect |
|---------|--------|
| `FT_MOTION` (M493) | Fixed-time trajectory planner with ZV/ZVD/MZV shaping; runtime toggle alongside Input Shaping |
| `FILAMENT_RUNOUT_SENSOR` | Pauses print and triggers M600 on runout (PA0 / Z-MIN connector) |
| `STARTUP_COMMANDS "M155 S2"` | TFT gets live temps immediately on boot |
| `POWER_LOSS_RECOVERY` | Resume after power failure |
| `ASSISTED_TRAMMING` (G35) | Guided manual bed screw leveling |
| `GCODE_MACROS` (M810–M819) | Runtime macro storage |
| `AUTO_REPORT_POSITION` (M154) | Auto-push coordinates to TFT |
| `PINS_DEBUGGING` (M43) | Pin state reporting and toggle |
| `SERIAL_XON_XOFF` | Software flow control |
...
| `PAREN_COMMENTS` | G-code `(inline comment)` support for Simplify3D and post-processors |
| `GCODE_QUOTED_STRINGS` | Quoted string parameters, e.g. `M117 "message"` |
| `FAN_KICKSTART_TIME 100` | 100 ms full-power burst to ensure fans spin up at any PWM level |
| Preheat temp broadcast fix | During M109/M190, temperatures broadcast to all serial ports (fixes TFT during USB host printing) |
