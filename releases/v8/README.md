# Marlin bugfix-2.1.x for Artillery Genius Pro — v8
...
**File:** `firmware-gpro-v8-0x08000000.bin`  
**Flash address:** `0x08000000`

### How to flash

1. Copy `firmware-gpro-v8-0x08000000.bin` to the root of a microSD card (FAT32, ≤32 GB)
...
**Fixed-Time Motion (FT Motion) — `M493`**

v8 adds FT Motion alongside the existing Input Shaping (M593). Both are compiled in; you choose at runtime:

- `M493 S1` — enable FT Motion (fixed-time trajectory with built-in ZV shaping at X=55 Hz, Y=48.6 Hz)
- `M493 S0` — return to standard motion (Input Shaping X/Y still active via M593)

Default shaper after `M493 S1`: **ZV** at X=55.0 Hz / Y=48.6 Hz (same frequencies as the M593 values).  
Compiled shapers: ZV, ZVD, MZV. Trajectory type: TRAPEZOIDAL at 1 kHz generation rate.

> The X/Y shaping frequencies were measured on a Sidewinder X2. Re-measure on your Genius Pro with an ADXL345 and update via `M493 A0 F<hz>` / `M493 A1 F<hz>`, then `M500`.
...
| File | Description |
|------|-------------|
| `firmware-gpro-v8-0x08000000.bin` | Pre-built binary, flash at 0x08000000 |
| `FEATURE_COMPARISON.md` | Full feature table vs stock / gpro-mp / mfagp references |
| `MERGE_REPORT.md` | Change log from v7 to v8 |
| `README.md` | This file |
